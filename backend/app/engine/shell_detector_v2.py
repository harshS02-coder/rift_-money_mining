...
class ShellAccountDetectorV2:
    ...
    def _calculate_comprehensive_stats(self) -> Dict[str, Dict]:
        ...
        for txn in self.transactions:
            ...
            # Record transaction
            stats[txn.sender_id]['transactions'].append(txn)
            stats[txn.sender_id]['outbound'].append(txn)
            stats[txn.sender_id]['total_out'] += txn.amount
            stats[txn.sender_id]['unique_destinations'].add(txn.receiver_id)
            stats[txn.sender_id]['timestamps'].append(txn.timestamp)
            stats[txn.sender_id]['amounts'].append(txn.amount)
            
            stats[txn.receiver_id]['transactions'].append(txn)
            stats[txn.receiver_id]['inbound'].append(txn)
            stats[txn.receiver_id]['total_in'] += txn.amount
            stats[txn.receiver_id]['unique_sources'].add(txn.sender_id)
            stats[txn.receiver_id]['timestamps'].append(txn.timestamp)
            stats[txn.receiver_id]['amounts'].append(txn.amount)
