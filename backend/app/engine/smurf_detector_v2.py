...
class SmurfingDetectorV2:
    ...
    def _analyze_window(self, transactions: List[Transaction], window_start: datetime) -> List[Dict]:
        ...
        for txn in transactions:
            account_txns[txn.sender_id].append(txn)
            account_txns[txn.receiver_id].append(txn)
        
        for account_id, txns in account_txns.items():
            if len(txns) >= 6:  # Minimum threshold
                # Calculate metrics
                fan_in = len(set(t.sender_id for t in txns if t.receiver_id == account_id))
                fan_out = len(set(t.receiver_id for t in txns if t.sender_id == account_id))
                
                sent = [t for t in txns if t.sender_id == account_id]
                received = [t for t in txns if t.receiver_id == account_id]
    ...
    def _detect_structuring_patterns(self) -> List[Dict]:
        ...
        for txn in self.transactions:
            account_amounts[txn.sender_id].append(txn.amount)
            account_amounts[txn.receiver_id].append(txn.amount)
    ...
    def _detect_consolidation(self) -> List[Dict]:
        ...
        for txn in self.transactions:
            account_flows[txn.receiver_id]['inbound'].append(txn.amount)
            account_flows[txn.sender_id]['outbound'].append(txn.amount)
    ...
    def _analyze_fan_patterns(self, min_in: int = 3, min_out: int = 3) -> List[Dict]:
        ...
        for txn in self.transactions:
            account_connections[txn.receiver_id]['sources'].add(txn.sender_id)
            account_connections[txn.sender_id]['destinations'].add(txn.receiver_id)
            account_volumes[txn.receiver_id] += txn.amount
            account_volumes[txn.sender_id] += txn.amount
    ...
    def _create_smurfing_alert(self, account_id: str, windows: List[Dict],
        ...
        if alert['transaction_count'] == 0 and alert['patterns']:
            account_txns = [t for t in self.transactions 
                          if t.sender_id == account_id or t.receiver_id == account_id]
