...
class SmurfingDetector:
    ...
    def _create_time_windows(self) -> Dict[str, List[Dict]]:
        ...
        for txn in sorted_txns:
            ...
            if txn.sender_id not in account_windows:
                account_windows[txn.sender_id] = []
            
            # Check if fits in existing window
            found_window = False
            for window in account_windows[txn.sender_id]:
            ...
            if not found_window:
                account_windows[txn.sender_id].append({
            ...
            if txn.receiver_id not in account_windows:
                account_windows[txn.receiver_id] = []
            
            found_window = False
            for window in account_windows[txn.receiver_id]:
                window_start = window["start_time"]
                window_end = window_start + timedelta(hours=self.window_hours)
            ...
            if not found_window:
                account_windows[txn.receiver_id].append({
                    "start_time": txn.timestamp,
                    "transactions": [txn],
                    "transaction_count": 1
                })
    ...
    def _calculate_window_metrics(self, account_id: str, window_data: Dict,
        ...
        fan_in = len(set(txn.sender_id for txn in transactions 
                        if txn.receiver_id == account_id))
        fan_out = len(set(txn.receiver_id for txn in transactions 
                         if txn.sender_id == account_id))
        
        # Calculate total amounts
        total_in = sum(txn.amount for txn in transactions if txn.receiver_id == account_id)
        total_out = sum(txn.amount for txn in transactions if txn.sender_id == account_id)
    ...
    def get_accounts_by_fan_activity(self, min_fan_in: int = 3, 
        ...
        for account_id, windows in time_windows.items():
            for window_data in windows:
                ...
                fan_in = len(set(txn.sender_id for txn in txns 
                               if txn.receiver_id == account_id))
                fan_out = len(set(txn.receiver_id for txn in txns 
                                if txn.sender_id == account_id))
    ...
    def detect_concentration_patterns(self) -> List[Dict]:
        ...
        for txn in self.transactions:
            # Track inbound
            if txn.receiver_id not in account_sources:
                account_sources[txn.receiver_id] = set()
            account_sources[txn.receiver_id].add(txn.sender_id)
            
            if txn.receiver_id not in account_volumes:
                account_volumes[txn.receiver_id] = 0
            account_volumes[txn.receiver_id] += txn.amount
            
            # Track outbound
            if txn.sender_id not in account_destinations:
                account_destinations[txn.sender_id] = set()
            account_destinations[txn.sender_id].add(txn.receiver_id)
